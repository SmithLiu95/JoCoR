...
class BNInception(nn.Module):
    ...
    def __init__(self, emd_size,num_classes=1000):
        ...
        self.last_linear = nn.Linear (emd_size, num_classes)

        self.fc_feat = nn.Linear(1024, emd_size)
        nn.init.kaiming_normal_(self.fc_feat.weight)
    ...
    def forward(self, input):
        ...
        return x
    def load_param(self):
        param_dict = torch.load('/home/liuchang/.torch/models/bn_inception-52deb4733.pth')
        for i in param_dict:
            if "last_linear" in i:
                continue
            self.state_dict()[i].copy_(param_dict[i])

def bninception(emd_size,num_classes=1000, pretrained=True):
    r"""BNInception model architecture from <https://arxiv.org/pdf/1502.03167.pdf>`_ paper.
    """
    model = BNInception(emd_size=emd_size,num_classes=num_classes)
    if pretrained:
